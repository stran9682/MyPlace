...
const ChatList = forwardRef<ChatListRef, ChatListProps>(function ChatList(
  { token, onSelectChat },
  ref
): ReactElement {
      ...
      {groups.length === 0 ? (
        ...
        <div className="chat-list-items">
          {groups.map((group) => (
            ...
            >
              <div className="chat-item-avatar">{group.groupName.charAt(0).toUpperCase()}</div>
              <div className="chat-item-content">
                ...
                {group.lastMessage ? (
                  <p className="chat-item-preview">
                    <span className="preview-username">{group.lastMessage.username}:</span>{' '}
                    {group.lastMessage.messageText}
                  </p>
                ) : (
                  <p className="chat-item-preview no-messages">No messages yet</p>
                )}
              </div>
            </div>
          ))}
